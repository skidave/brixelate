...
class PrintEstimate(object):
	...
	def __init__(self, context):
		...
		target_objects = [ob for ob in self.scene.objects if not ob.name.startswith('~COPY~') and not ob.name.startswith('~HOLLOW~') and not re.search(r"Plane", ob.name) and re.match(r"[BP]_\dx\d", ob.name) is None]
		estimates = {}
		total_print_time = 0
